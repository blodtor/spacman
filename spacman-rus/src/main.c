 ...
 * Pac-Man для Sega Mega Drive / Sega Genesis
 *
 * Игра для двух игроков, включая возможность игры по сети на двух приставках
 * через SEGA Link Cable (Zero Tolerance Link Cable), подключаемый во второй порт.
 *
 * Посмотреть, что такое SEGA Link Cable на примере Zero Tolerance и Super Turbo MEGA Pac-Man, можно тут:
 * https://youtu.be/aLk8tDrjZ50
 *
 * Для игры был реализован SEGA Link Cable Protocol (LCP) - протокол для обмена данными через SEGA Link Cable
 * между двумя приставками Sega Mega Drive / Sega Genesis. Это библиотека на языке программирования Си для SGDK 2.12
 *
 * Описание игры:
 * 1-й игрок управляет Pac-Man, за которым гоняется 1 призрак красного цвета.
 * В случае если призрак догонит Pac-Man - игра закончится.
 * В случае если Pac-Man или Pac-Girl съедят зелёную точку, призрак станет
 * фиолетового цвета и начнёт убегать от Pac-Man, так как теперь его самого можно съесть.
 * 2-й игрок управляет Pac-Girl. Она помогает собирать точки на уровне, при
 * этом ни Pac-Man, ни призрак не могут съесть Pac-Girl, а она их (бессмертная).
 * Игра закончится победой, если игроки смогут собрать всю еду (белые и зелёные точки) на уровне.
 * В доме призраков через некоторое время появляется вишня - за неё просто дают очки,
 * чтобы выиграть, есть её не обязательно!
 * В меню выбора количества игроков можно выбрать, кто каким персонажем будет играть
 * при игре вдвоём. Для этого надо нажать вправо в меню '2 PLAYERS', чтобы первый игрок
 * управлял Pac-Girl, а второй игрок - Pac-Man, или влево, чтобы вернуть настройки по умолчанию.
 *
 ...
 * [папка, где лежит проект]/spacman/spacman-rus/out/rom.bin
 * 
 * SGDK - свободный и открытый development kit для Sega Mega Drive / Sega Genesis
 * https://github.com/Stephane-D/SGDK.git
 *
 * SGDK_wine - Wine wrapper скрипт, генерирующий /opt/SGDK/makefile_wine.gen для SGDK, чтобы использовать его в Linux
...
#error "Set MODULE_LINK_CABLE to 0 in config.h and rebuild the library"
#endif
 ...
 * другой объект синхронизации OBJECT_TYPE_SLAVE с другой фразой "Pac-Man!"
 */
 ...
 * другой объект синхронизации OBJECT_TYPE_MASTER с другой фразой "Pac-Girl"
 */
...
/**
 * Получаем dx, dy, pacmanX, pacmanY, oldX, oldY из байтового массива transferObject
...
void pacManStateFromTransferObject() {
	...
	if (transferObject[1] & 0b01000000) {
		// если 6 бит = 1, то двигаемся вниз
		dy = 1;
	} else if (transferObject[1] & 0b10000000) {
		// если 7 бит = 1, то двигаемся вверх
		dy = -1;
	} else {
		// иначе стоим на месте
		dy = 0;
...
/**
 * Сохраняем dxPacGirl, dyPacGirl, pacGirlX, pacGirlY, oldPacGirlX, oldPacGirlY в байтовом массиве transferObject
...
/**
 * Получаем dxPacGirl, dyPacGirl, pacGirlX, pacGirlY из байтового массива transferObject
...
void pacGirlStateFromTransferObject() {
    ...
    // в новых координатах отображаем PACGIRL
	map[pacGirlY][pacGirlX] = PACGIRL;
}


/**
 * Сохраняем dxRed, dyRed, redX, redY, redFlag в байтовом массиве transferObject
...
/**
 * Получаем dxRed, dyRed, redX, redY, redFlag из байтового массива transferObject
...
void redStateFromTransferObject() {
	...
	if (transferObject[0] & 0b00100000) {
		// если 5 бит == 1, то призрак не съедобный
		redFlag = 1;
...
/**
 * В transferObject сохраняем объект, содержащий информацию о том, что было нажато на первом контроллере
...
void padToTransferObject(u16 pad) {
	...
	transferObject[1] = pad & 0x00FF; // младший байт из pad во 2-й элемент массива, что передадим
}

/**
 * Из объекта, который лежит в transferObject, переданного через Link Cable, получаем информацию о том, что было нажато
 ...
 * смотрим, что подключено во 2 порт контроллера.
 * Если играем через SEGA Link Cable, определяем, какая
 * приставка ведущая (master), а какая ведомая (slave)
 */
void initControllerPort2() {
	// неизвестно, есть ли соединение между приставками
    controllerPort2Mode = MODE_PORT2_UNKNOWN;

    // заполняем нулями текст типа игры
    memset(gameModeText, 0, GAME_MODE_TEXT_SIZE + 1);

	// определяем, что подключено во второй порт приставки
	u8 pad2type = JOY_getJoypadType(JOY_2);
	if (pad2type == JOY_TYPE_PAD3 || pad2type == JOY_TYPE_PAD6) {
		// подключен 3 или 6 кнопочный контроллер во 2 порт приставки
		controllerPort2Mode = MODE_MULTI_PLAYER;

		// вывести на экран тип игры '2 ИГРОКА НЕ СЕТЕВАЯ! ' - 2 player играет на контроллере, подключенном
		// во 2 порт нашей же приставки, НЕ через SEGA Link Cable
		memcpy(gameModeText, TEXT_2P_NO_LINK, GAME_MODE_TEXT_SIZE);

		return;
	} else if (pad2type == JOY_TYPE_UNKNOWN) {
		// сброс ошибок при передаче данных в 0, которые показываем на экране через Link cable
		linkCableErrors = 0;
		// сброс количества ошибок при передаче данных через Link cable
		linkCableErrorsCount = 0;
		// сброс количества отрисованных фреймов с начала создания соединения через Link cable
		...
		do {
			...
			// и получить от нее пакет LCP_ReceivePacke с данными
			...
			// выходим из цикла! Надо смотреть, что было получено в LCP_ReceivePacke! Возможно наша приставка будет ведущей (master)
			...
			// попытаться обменяться данными еще раз!

			linkCableErrors = LCP_getError();
		} while (!( linkCableErrors == 0x1A || linkCableErrors == 0));

		// если ошибок при передаче и получении пакетов не было,
		// то LCP_getNextObjectFromReceivePacket() вернет объект, полученный от другой приставки
		while ((objectType = LCP_getNextObjectFromReceivePacket(transferObject, LINK_TYPES_LENGTH))) {
			...
			if (objectType == OBJECT_TYPE_SLAVE && LCP_getError() == 0) {
				// Наша приставка становится ведущей (master)
				// это значит далее при взаимодействии мы вызываем внешнее прерывание - External interrupt (EX-INT)
				// у другой приставки, т.к. она будет ведомой (slave) отправляем пакет, а затем получаем пакет.
				// теперь отправка и получение пакетов на нашей приставке с помощью метода LCP_masterCycle()
				// на нашей приставке не будет вызываться внешнее прерывание, работаем синхронно из основного кода игры
				...
				// вывести на экран тип игры 'Pac-Man!' - 2 player при этом играет на контроллере, подключенном
				// в 1 порт другой приставки через SEGA Link Cable, подключенный во 2 порт обоих приставок

				memcpy(gameModeText, TEXT_LINK_MASTER, GAME_MODE_TEXT_SIZE);

				// звук удалось создать соединение по Link cable
				XGM_startPlayPCM(SFX_SOUND_CONNECT_LINK_CABLE, 15, SOUND_PCM_CH4);

				// надо немного подождать, чтобы ведомая приставка (slave) успела обработать полученный пакет
				SYS_doVBlankProcess();
		...
		// и в случае получения нашей приставкой пакета от другой мы ей отправим оба объекта в пакете
		// но ведущая приставка будет ожидать только OBJECT_TYPE_SLAVE, объект OBJECT_TYPE_MASTER будет ей
		// проигнорирован, смотрите код выше

		// открываем 2 порт приставки, что означает теперь наша приставка будет обрабатывать внешние прерывания
		// External interrupt (EX-INT) и сначала получать пакеты при наступлении этого прерывания, затем отправлять.
		// Происходить это будет асинхронно с основным кодом игры, потому что по сути другая приставка
		// удаленно вызывает метод нашей приставки LCP_slaveCycle(), останавливая текущее выполнение программы на нашей
		// в любой случайный момент
		LCP_open();

		// таймер, по истечении которого считаем, что нет соединения по SEGA Link Cable
		u16 timer = 0xFFFF;
		while (timer > 0) {
			drawText();
			SYS_doVBlankProcess();
			...
			// пытаемся получить объект из полученного пакета
			...
			if (objectType == OBJECT_TYPE_MASTER && linkCableErrors == 0) {
				// Наша приставка становится ведомой (slave)
				// это значит что далее при взаимодействии ведущая приставка (master) другого игрока
				// будет вызывать у нашей внешнее прерывание - External interrupt (EX-INT) в следствии
				// чего основной код игры на нашей ведомой приставке (slave) будет приостановлен на время
				// выполнения функции обработчика внешнего прерывания.
				// теперь получение и отправка пакетов на нашей приставке будет происходить асинхронно
				// посредством вызова метода LCP_slaveCycle() в любой случайный для нас момент другой приставкой,
				...
				// вывести на экран тип игры 'Pac-Girl' - 1 player при этом играет на контроллере, подключенном
				// в 1 порт другой приставки через SEGA Link Cable, подключенный во 2 порт обоих приставок
				memcpy(gameModeText, TEXT_LINK_SLAVE, GAME_MODE_TEXT_SIZE);

				// звук, что удалось создать соединение по Link cable
				XGM_startPlayPCM(SFX_SOUND_CONNECT_LINK_CABLE, 15, SOUND_PCM_CH4);
	...
	// Нет соединения по SEGA Link cable и во 2-й порт не включен 3 или 6 кнопочный контроллер
...
/**
 * Подсчет очков с учетом всех бонусов
 */
void calcScore() {
	u16 i;
	score100 = food100;
	...
	for (i = 0; i < redBonus; i++) {
		// 50 очков за съеденного призрака
		score010 += 5;
 ...
 * значения передаются по ссылке, поэтому они меняются
 */
 ...
 * если вишню не съел PACMAN, она появится опять
 */
 ...
 * где будет еда и пауэр-апы
 */
void resetGame() {
	u16 i, j;
	// счетчики циклов начинаются с разных значений,
	// чтобы рендеринг каждого персонажа был в разном глобальном цикле
	...
	// расставляем еду по карте (серые точки)
	...
	// Надо сбросить, т.к. когда начнем новую игру, можем еще не успеть получить
 ...
 * и что было съедено на месте призрака: PACMAN или PACGIRL
 */
u8 pacmanLooser() {
	...
	if (redY == pacmanY && redX == pacmanX) {
		...
		if (redFlag) {
			...
			// Отправляем его в дом Привидений
			redY = 10;
			redX = 15;
			// обездвиживаем
			dyRed = 0;
			...
			// проверяем, что Pac-Man съел вместе с RED
			if (oldRedVal == FOOD) {
				// еду
				incFood();
				...
				// звук поедания пауэр-апа
				XGM_startPlayPCM(SFX_SOUND_POWERUP, 15, SOUND_PCM_CH2);

				// обновляем время, когда RED стал съедобным
				redTime = RED_TIME;

		    	if (MODE_PORT2_MASTER == controllerPort2Mode) {
		    		// отправляем ведомой (slave) приставке, что был съеден power-up (зеленая точка)
 ...
 */
u8 pacManState() {
	// проверяем, задано ли PACMAN направление движения
	if (dx != 0 || dy != 0) {
		...
		if (pacmanLastUpdateTime == 0) {
			...
			// корректируем координаты PACMAN, если надо (чтобы не вышел с поля)
			// если вышел за поле (появление с другой стороны поля)
			moveBound(&pacmanX, &pacmanY);

			// если текущая клетка с едой, увеличиваем счетчик съеденного
			...
			// если съедены все FOOD и POWER_FOOD - PACMAN выиграл
			if (winner()) {
			...
			// съел ли PACMAN призрака (или оно нас)
			if (pacmanLooser()) {
 ...
 */
u8 pacGirlState() {
	// проверяем, задано ли у pacGirl направление движения
	if (dxPacGirl != 0 || dyPacGirl != 0) {
		...
		if (pacGirlLastUpdateTime == 0) {
			...
			if (isNotWallOrDoor(pacGirlY, pacGirlX)) {
				...
				// вектор движения сбрасываем (PAC-GIRL останавливается)
				dxPacGirl = 0;
				dyPacGirl = 0;
			}
...
u8 redState() {
	...
	// проверяем, задано ли у RED направление движения
	if (dxRed != 0 || dyRed != 0) {
		if (redLastUpdateTime == 0) {
			...
			// меняем координаты призрака (одновременно dxRed и dyRed не могут быть больше 0!)
			redX = redX + dxRed;
			redY = redY + dyRed;
			...
			if (isNotWall(redY, redX)) {
				// текущие координаты не препятствие
				...
				if (redX == 15 && redY >= 7 && redY <= 10) {
					...
					if (redFlag && redX != pacmanX) {
						...
						if (isNotWallOrDoor(redY, redX + 1)	&& isNotWallOrDoor(redY, redX - 1)) {
							...
							// слева есть альтернативный путь для призрака
							if (abs(redX - 1 - pacmanX) < abs(redX - pacmanX)) {
								// путь влево короче до Pac-Man по X
								dxRed = -1;
							}
				...
				// текущие координаты у призрака - препятствие, надо изменить направление движения
				if (redX == 15 && redY >= 7 && redY <= 10) {
					// из дома призрака выходит он всегда вверх
					dyRed = -1;
					dxRed = 0;
				} else {
					// возвращаем призрака в предыдущие координаты
					redX = oldXRed;
					redY = oldYRed;
 ...
 *  SEGA
 *
 *  Перерисовать на бэкграунде tile, когда съели точку
 *  рисуем черный квадрат 8x8
 */
void drawBlackBox(s16 y, s16 x) {
	// отображаем пустой квадрат из Русских Tile
...
void drawRussianText(VDPPlane plane, const u8 *vramOffsets, u8 length, u8 xTile, u8 yTile) {
	// получаем размер plane по горизонтали
    ...
    if ((xTile >= pw) || (yTile >= ph)) {
    	// не нужно рисовать текст в невидимой области экрана
        return;
    }
    ...
    // если строка полностью не влезет на экране, обрезаем ее
    // на столько, чтобы влезла
    if (n > (pw - xTile)) {
        n = pw - xTile;
    }
 ...
 * 4 символа типа char, число выводится в шестнадцатеричном формате
 * например, десятичное число 65535 будет выведено как FFFF
 *
 * val - число, которое хотим вывести на экран
 */
 ...
 *  результат игры ('КОНЕЦ ИГРЫ' или ТЫ ВЫИГРАЛ')
 */
void drawText() {

	switch (showLinkCableErrors) {
		case SHOW_LINK_CABLE_LAST_ERROR:
			// выводим на экран ошибки при передаче через Link Cable
			printU16(linkCableErrors);
			VDP_drawText(text, 28, 25);
		break;

		case SHOW_LINK_CABLE_ERRORS_COUNT:
			// выводим на экран количество ошибок при передаче через Link Cable
			printU16(linkCableErrorsCount);
			VDP_drawText(text, 28, 25);
		break;

		case SHOW_LINK_CABLE_FRAME_COUNT:
			// количество отрисованных фреймов с начала создания соединения через Link Cable
	...
	if (STATE_SCREENSAVER != gameState) {
		...
		// "ВЕДОМАЯ ПРИСТАВКА   " - (TEXT_LINK_SLAVE) игра через Link Cable на двух приставках, и наша приставка ведомая (slave)
		// "2 ИГРОКА НЕ СЕТЕВАЯ! " - (TEXT_2P_NO_LINK) игра не через Link Cable, и во 2 порту подключен контроллер, играет 2 игрока на одной приставке
		// "1 ИГРОК НЕ СЕТЕВАЯ!  " - (TEXT_1P_NO_LINK) игра не через Link Cable, и во 2 порту нет контроллера, играет 1 игрок
	...
	if (STATE_GAME == gameState || STATE_RESULT == gameState) {
        ...
        // количество съеденных пауэр-апов
		text[0] = powerBonus + '0';
	...
	switch (gameState) {
		...
		case STATE_SELECT:
			...
			if (players == 1) {
				...
				// если выбрана игра на двух игроков
		...
		case STATE_PAUSE:
			...
			// нарисовать спрайт со словом PAUSE
			SPR_setAnim(pauseSprite, 0);
			SPR_setPosition(pauseSprite, pauseX, pauseY);

			// нарисовать спрайт бегущего Соника
...
void drawSprite(s16 i, s16 j, u8 val) {
    // x = j * 8 и на 29 пикселов вправо
    x = (j << 3) + 29;

    // y = i * 8 и на 2 пиксела вверх
    y = (i << 3) - 2;
    ...
    if (val == PACMAN) {
		...
		// логика для воспроизведения звука, когда можно есть призрака
 ...
 * SEGA
 *
 * Обновить карту, персонажей, двери, вишню на экране
 */
void refreshGame() {
...
void actionsStateScreensaver() {
	...
	// просто закончилась анимация: Pac-Girl X координата >= 380
	...
	if ((pacGirlX >= 380)
		...
		// музыка, играющая при отображении меню выбора количества игроков
 ...
 * SEGA
 *
 * Обработка нажатых кнопок игроками и логика игры во время отображения экрана с меню выбора количества игроков
...
void actionsStateSelectPlayers() {
	...
	if (((pad1 & BUTTON_START) || (pad2 & BUTTON_START)) && playersTime == 0) {
		...
		if (players != 2) {
		    ...
		    // первый игрок всегда в одиночную игру управляет Pac-Man
			switchPlayers = P1_PACMAN__P2_PACGIRL;

			// сбрасываем всё, что нажато на 2 контроллере
			pad2 = 0;
			
			// выводим на экран что соединения с другой приставкой нет, играем в одного
			memcpy(gameModeText, TEXT_1P_NO_LINK, GAME_MODE_TEXT_SIZE);

			if ((MODE_PORT2_MASTER == controllerPort2Mode || MODE_PORT2_SLAVE == controllerPort2Mode)) {
				// устанавливаем режим работы второго порта - не участвует в игре, игрок нашей приставки
				// играет в одного на своей приставке контроллером подключенным в первый порт
				controllerPort2Mode = MODE_SINGLE_PLAYER;

				// есть соединение по Link cable
				// закрываем порт, данные больше не будут пересылаться через Link cable
				LCP_close();
			...
			// выбрана игра на 2ых (2 PLAYERS)

			if (!(MODE_PORT2_MASTER == controllerPort2Mode || MODE_PORT2_SLAVE == controllerPort2Mode)) {
				// если еще не определили что у нас воткнуто во 2 порт приставки
				// или это режимы отличные от игры в 2х по Link cable
			...
			if (controllerPort2Mode == MODE_SINGLE_PLAYER) {
				...
				// нет соединения через Link cable
		...
		// повторное нажатие разрешено только через 30 кадров (чтобы сразу не нажалась пауза)
		playersTime = 30;
		return;
	}
...
void actionsStateGame() {
	if (MODE_PORT2_SLAVE != controllerPort2Mode) {
		// если не ведомая приставка (slave), то нужно, чтобы отработала логика игры
		if (((pad1 & BUTTON_START) || (pad2 & BUTTON_START)) && playersTime == 0) {
		...
		if (pad1 &  BUTTON_LEFT) {
			// нажата кнопка влево на 1 джойстике
			dx = -1;
			dy = 0;
		}
		...
		if (cherryTime > 0) {
			// счетчик, когда надо показать вишню
			// и открыть к ней дверь
			--cherryTime;
		}
 ...
 * SEGA
 *
 * Обработка нажатых кнопок игроками и логика игры при показе экрана с результатом после окончания игры
 * для gameState == STATE_RESULT
 */
void actionsStateResult() {
	if ((pad1 & BUTTON_START) || (pad2 & BUTTON_START)) {
		// показываем результат игры на этом экране
		// нажат Start на 1 или 2 джойстике
		// переходим на экран заставки
		gameState = STATE_SCREENSAVER;
		// защита от повторного нажатия кнопки Start
		playersTime = 30;
		// нужно всех персонажей убрать с экрана
		// для этого ставим их в начальное состояние перед запуском заставки
		initScreensaver();
...
void actions() {
	...
	if (playersTime > 0) {
		...
		// когда playersTime станет равным 0, обработчик заработает снова
		--playersTime;
	}
 ...
 * SEGA
 *
 * сбрасываем значения переменных для отображения заставки
...
void screensaver() {
   ...
   // отобразить по горизонтали спрайт RED
   SPR_setHFlip(redSprite, TRUE);
 ...
 * отправляем ведомой (slave) приставке что было нажато на нашем контроллере
 ...
 * затем разбираем что было получено от ведомой (slave) приставки а точнее что было нажато на контроллере
 * другой приставки
 */
void masterControls() {
	...
	// передаем пакет с данными от нашей ведущей приставки (master) ведомой приставке (slave)
	...
	do {
		...
		if (OBJECT_TYPE_JOY == objectType) {
			...
			// поэтому что было нажато на первом контроллере другой приставки сохраняем в pad2
 ...
 * сохраняем в пакет для отправки ведущей (master) приставке что было нажато на нашем контроллере
 ...
 * разбираем что было получено от ведущей (master) приставки при прошлом вызове внешнего прерывания (LCP_slaveCycle())
 */
void slaveControls() {
	// мы играем за второго игрока, поэтому что нажато на первом контроллере
	...
	if (pad1) {
		...
		// в момент получения внешнего прерывания вызванного ведущей приставкой (master) в случайный момент времени для нас
		LCP_objectToPacketForSend(transferObject, OBJECT_TYPE_JOY, LINK_TYPES_LENGTH);
	}

	// считаем что на 1 контроллере на данный момент ничего не нажато
	pad1 = 0;
	...
	do {
		...
		switch (objectType) {
			case OBJECT_TYPE_JOY:
				...
				// поэтому что было нажато на первом контроллере другой приставки сохраняем в pad1
			...
			case OBJECT_TYPE_EAT_CHERRY:
				// ведущая приставка (master) сообщила что cъедена вишня
				// звук поедания вишни
				XGM_startPlayPCM(SFX_SOUND_CHERRY, 15, SOUND_PCM_CH2);
 ...
 * было нажато на первом контроллере подключенном у другой приставке.
 * Анализ объектов в пришедших пакетах по Link Cable, тоже происходит тут и для ведущей (master) и
 * для ведомой (slave) приставки.
 *
 * pad1 - игрок управляющий Pac-Man. Что нажато на 1 контроллере при игре на одной приставке.
 ...
 *
 * pad2 - игрок управляющий Pac-Girl. Что нажато на 2 контроллере при игре на одной приставке.
 *        При игре через Link Cable по умолчанию в pad2 будет что нажато на ведомой приставке (slave)
 *        на 1 контроллере, так же игра за Pac-Girl
 *
 * НО если в меню '2 PLAYERS' нажать вправо то pad1 и pad2 поменяются местами! Т.е. когда
 * switchPlayers == 1 и players == 2 игроки будут играть за противоположенных персонажей!
 *
 */
void controls() {
	// буферная переменная используется в случае если игроки поменяли кто кем будет играть в меню '2 PLAYERS'
	// нужна чтобы поменять местами pad1 и pad2
	u16 switchPad;
	...
	switch (controllerPort2Mode) {
		case MODE_MULTI_PLAYER:
			// multiplayer нет соединения между приставками по Link Cable но во втором порту 3 или 6
			// кнопочный контроллер. Второй игрок по умолчанию играет за PAC-GIRL этим контроллером
		...
		case MODE_PORT2_MASTER:
			// master - наша приставка ведущая, игра вдвоем через Link Cable (сетевая игра на двух приставках SEGA)
			// играем за первого игрока PAC-MAN но если в меню '2 PLAYERS' сменили персонажей то за PAC-Girl

			masterControls();
		break;
		case MODE_PORT2_SLAVE:
			// slave - наша приставка ведомая, игра вдвоем через Link Cable (сетевая игра на двух приставках SEGA)
			// играем за второго игрока PAC-GIRL но если в меню '2 PLAYERS' сменили персонажей то за PAC-MAN

			slaveControls();
		break;
		case MODE_SINGLE_PLAYER:
			// singleplayer - нет соединения через Link Cable и в 2 порту нет контроллера
		  	// основной и единственный игрок тот кто нажимает кнопки на первом контроллере, поэтому
			// ничего не делаем мы и так уже все положили в pad1
		case MODE_PORT2_UNKNOWN:
			// не знаем что со вторым контроллером,
			// ничего не делаем
		default:
			// не нужно обрабатывать события от 2-го порта контроллера
			// ничего не делаем
		break;
	}
...
void initSprites() {
    ...
    // добавляем спрайт вишни на экран
    ...
    // добавляем спрайт паузы на экран
...
void initGame() {
    ...
    // сбрасываем режим работы приставки со вторым портом для контроллера
    controllerPort2Mode = MODE_PORT2_UNKNOWN;

    // ничего не отображаем на экране по поводу режима игры
    memset(gameModeText, 0, GAME_MODE_TEXT_SIZE);

    // по умолчанию первый игрок управляет Pac-Man, второй игрок Pac-Girl
    switchPlayers = P1_PACMAN__P2_PACGIRL;

    // по умолчанию выбрана одиночная игра
    players = 1;
...
int main() {
	...
	while(1) {
		...
		// ведомой приставке (slave) и реакция на полученные объекты по Link Cable
		controls();
		...
		// подвинуть персонажей в зависимости от того что нажато на карте (map)
		actions();
		...
		// количество отрисованных фреймов
		++linkCableFrameCount;
	}
